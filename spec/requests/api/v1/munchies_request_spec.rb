...
describe 'Munchies API' do
  ...
  context 'happy path' do
    it 'sends weather data for restaurant location' do
      ...
      expect(destination[:data][:type]).to eq("munchies")
      ...
      expect(destination[:data][:attributes][:forecast]).to have_key(:summary)
      expect(destination[:data][:attributes][:forecast]).to have_key(:temperature)
